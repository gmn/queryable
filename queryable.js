    ...
    function db_object(config) {
        ...
        function finish_db_setup() {
            if (this.master.length > 0) {
                ...
                this.master = this.master.sort(function (a, b) {
                    return a._id - b._id
                });
            }
        } // finish_db_setup
    ...
    db_object.prototype = {
        ...
        save: function (arg1, arg2) {
            ...
            if (this.platform === "node_module") {
                ...
                try {
                    if (pretty) {
                        fs.writeFileSync(this.db_path, JSON.stringify(this.master, null, 2), {
                            encoding: "utf8",
                            mode: mode,
                            flag: 'w'
                        });
                    } else {
                        fs.writeFileSync(this.db_path, JSON.stringify(this.master), {
                            encoding: "utf8",
                            mode: mode,
                            flag: 'w'
                        });
        ...
        distinct: function (str, clause, callback) {
            ...
            for (var i = 0, l = maybe.length; i < l; i++) {
                if (!(distinct_set.some(function (x) {
                        return x[str] === maybe[i][str]
                    })))
                        ...
                        if (row.hasOwnProperty(key) && key === test.key) {
                            switch (cond) {
                                ...
                                case '$ne':
                                    ...
                                    break;
                                case '$in':
                                    if (type_of(test.value[cond]) === 'array') {
                                        var in_array = test.value[cond];
                                        for (var in_index = 0; in_index < in_array.length; in_index++) {
                                            if(row[key] === in_array[in_index]) {
                                                res.push(row);
                                                continue next_row;
                                            }
                                        }
                            ...
                            case "CLAUSE_CONDITIONAL":
                                switch (_firstKey(test.value)) {
                                    ...
                                    case '$exists':
                                        ...
                                        break;
                                    case '$in':
                                        if (type_of(test.value['$in']) === 'array') {
                                            var cin_array = test.value['$in'];
                                            for (var cin_index = 0; cin_index < cin_array.length; cin_index++) {
                                                if(row[test.key] === cin_array[cin_index]) {
                                                    res.push(row);
                                                    continue next_row;
                                                }
                                            }
    ...
    queryable.open = function (config) {
        ...
        function server_open(config) {
            ...
            return new db_object({
                db_path: that.db_path,
                db_dir: that.db_dir,
                db_name: that.db_name,
                "platform": that.platform,
                use_gzip: that.use_gzip,
                data: rows
            });
        } // server_open()
